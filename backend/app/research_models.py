...
class MoltbookInteractionFeedback(Base):
    __tablename__ = "moltbook_interaction_feedback"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    post_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    comment_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    rating: Mapped[str] = mapped_column(String(20), nullable=False)  # up/down/no_vote
    source: Mapped[str] = mapped_column(String(30), nullable=False, default="ai_judge")
