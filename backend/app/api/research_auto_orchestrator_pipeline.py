...
async def _run_v11_2_auto() -> Any:
    """Run v11.2 automatically for compatible completed experiments.

    No experiment IDs are hardcoded. The current experiment is discovered by
    matching a 2026 unseen-year result to a 2024/2025 baseline in the same
    mission. Existing v11.2 results are reused when their source result IDs
    have not changed.
    """
    from app.research_models import ExperimentResult
    ...
    try:
        ...
        for exp in experiments:
            ...
            for row in results:
                ...
                if "v11.2 three-year robustness gate" in method:
                    continue
                periods = m.get("periods") or {}
                if periods.get("unseen") == "2026" or "2026" in str(m.get("summary", "")):
                    candidates.append((exp, row))
                    break

        runs = []
        for current_exp, current_result in candidates:
            baselines = []
            for exp in experiments:
                if exp.id == current_exp.id:
                    continue
                if getattr(exp, "mission_id", None) != getattr(current_exp, "mission_id", None):
                    continue
                results = (
                    db.query(ExperimentResult)
                    .filter(ExperimentResult.experiment_id == exp.id)
                ...
                for row in results:
                    ...
                    periods = m.get("periods") or {}
                    method = str(m.get("method", ""))
                    if periods.get("is") == "2024" and periods.get("oos") == "2025" and "v11.2" not in method:
                        baselines.append((exp, row))
                        break
            if not baselines:
                continue

            baseline_exp, _ = baselines[0]
            ...
            for row in existing:
                try:
                    ...
                    continue
                if "v11.2 three-year robustness gate" in str(m.get("method", "")) and str(m.get("source_result_id")) == source_id:
                    already_done = True
                    break
            if already_done:
...
async def run_cycle() -> dict[str, Any]:
    async with _lock:
        ...
        try:
            ...
            try:
                cycle.append({"stage": "v11_2_three_year", "status": "completed", "result": await _run_v11_2_auto()})
            except Exception as exc:
                cycle.append({"stage": "v11_2_three_year", "status": "error", "error": f"{type(exc).__name__}: {exc}"})
