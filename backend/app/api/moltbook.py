...
def _result_stage_rank(row: ExperimentResult) -> int:
    """Return the research stage represented by an ExperimentResult.

    v11.2 MUST be detected before v11 because v11.2 metrics can also contain
    sizing-related fields. The highest rank is the publication source of truth.
    """
    ...
    # v11.2 / three-year robustness gate MUST win over v11.
    if (
        "v11.2" in method_text
        or "three-year" in method_text
        or "three_year" in metrics_text
        or "three-year" in metrics_text
        or "robustness gate 3-year" in summary
        or "robustness gate 3-year" in conclusion
    ):
        return 112
    ...
    if (
        ...
        or (
            ...
            and "sizing_context" in json.dumps(metrics.get("experiment_scope", {})).lower()
        )
...
def _build_post(
    ...
    if is_v11_sizing:
        ...
        if _result_stage_rank(result) >= 112:
            lines.extend([
                "",
                "Final research gate:",
                "v11.2 — Three-Year Robustness Gate",
            ])
...
async def publish_research(experiment_id: str, republish: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "selected_result_id": str(getattr(result, "id", "")),
