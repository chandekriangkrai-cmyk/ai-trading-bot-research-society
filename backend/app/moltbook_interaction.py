...
# ==================== V42.6 AI THREAD REPLY ====================

def _v42_6_flatten_comments(value, out=None):
    """Flatten Moltbook comment trees so nested replies are not missed."""
    if out is None:
        out = []
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            if key in {
                ...
                _v42_6_flatten_comments(child, out)

    elif isinstance(value, list):
        for item in value:
            _v42_6_flatten_comments(item, out)

    return out


def _v42_6_comment_text(comment):
...
def _v42_6_author_name(comment):
    if not isinstance(comment, dict):
        return ""
    ...
    for key in ("author_name", "username", "user_name", "author"):
        value = comment.get(key)
...
async def _v42_6_openrouter_reply(thread_text: str) -> str:
    """Generate one conversational reply with the configured OpenRouter model."""

    api_key = (
        os.getenv("OPENROUTER_API_KEY")
        or os.getenv("RESEARCH_AI_API_KEY")
        or os.getenv("AI_KEY")
        or ""
    ).strip()
    ...
    if not api_key:
        raise HTTPException(
            status_code=503,
            detail="OpenRouter/Research AI API key is not configured",
        )

    model = (
        os.getenv("AI_MODEL")
        or os.getenv("RESEARCH_AI_MODEL")
        or "google/gemini-3.8-flash"
    ).strip()

    prompt = f"""
You are an AI research agent continuing a technical conversation on Moltbook.

Read the entire thread below.

Your task:
- Identify the latest substantive point made by another agent after our previous contribution.
- Continue the conversation naturally.
- Answer their point directly.
- Stay technical and evidence-oriented.
- Do not pretend to have run experiments that were not provided.
- Do not repeat our previous question.
- Ask one useful follow-up question only if it genuinely advances the research.
...
""".strip()
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You are a concise research-discussion agent. "
                    "Respond to the other agent's latest technical point."
                ),
            },
            {
                "role": "user",
    ...
    headers = {
        ...
        "Content-Type": "application/json",
        "HTTP-Referer": "https://www.moltbook.com",
...
async def _v42_6_moltbook_request(
    method,
    url,
    payload=None,
):
...
async def _v42_6_verify_comment_response(published):
    """
    Reuse the existing verification solver when available.
    V42.6 does not create a second AI request for verification.
    """
    ...
    if not isinstance(published, dict):
        return {
            "attempted": False,
            "verified": False,
            "reason": "publish response is not an object",
        }

    candidates = [
        published,
        published.get("comment"),
        published.get("data"),
    ...
    for item in candidates:
        ...
        comment_id = comment_id or item.get("id")

        candidate = item.get("verification")
    ...
    if not verification:
        return {
            "attempted": False,
            "verified": True,
            "reason": "no verification challenge returned",
    ...
    if not challenge or not code:
        return {
            "attempted": False,
            "verified": False,
            "reason": "verification challenge/code missing",
            "comment_id": comment_id,
        }

    solver = globals().get("_solve_challenge")

    if not callable(solver):
        return {
            "attempted": False,
            "verified": False,
            "reason": "existing challenge solver is unavailable",
            "comment_id": comment_id,
        }

    try:
        answer = solver(challenge)
    except Exception as exc:
        return {
            "attempted": False,
            "verified": False,
            "reason": f"challenge solver failed: {exc}",
            "comment_id": comment_id,
            "challenge_text": challenge,
        }

    status, verify_body = await _v42_6_moltbook_request(
        "POST",
    ...
    return {
        ...
        "verified": (
            ...
            and verify_body.get("success", True) is True
        ),
        "answer": answer,
        "comment_id": comment_id,
        "http_status": status,
...
@router.post("/reply-thread-auto-verify")
async def v42_6_reply_thread_auto_verify(
    post_id: str,
) -> dict[str, Any]:
    """
    Read one Moltbook thread, generate one AI reply to the latest
    non-self contribution, post it, and attempt verification.

    One AI generation request is used per execution.
    """
    ...
    # Fetch the thread.
    status, body = await _v42_6_moltbook_request(
        "GET",
    ...
    if status >= 400:
        raise HTTPException(
            status_code=502,
            detail={
                "message": "Could not read Moltbook thread",
                "http_status": status,
                "response": body,
            },
        )

    comments = _v42_6_flatten_comments(body)
    ...
    own_name = (
        os.getenv("MOLTBOOK_AGENT_NAME")
        or os.getenv("MOLTBOOK_USERNAME")
        or os.getenv("AI_AGENT_NAME")
        or "aitradingresearch"
    ).strip().lower()

    normalized = []

    for comment in comments:
        author = _v42_6_author_name(comment)
        content = _v42_6_comment_text(comment)
            ...
            normalized.append(
                {
                    ...
                    "author": author,
                    "content": content,
                    "is_self": author.strip().lower() == own_name,
                    "created_at": (
    ...
    if not normalized:
        return {
            "status": "no_action",
            "reason": "no_readable_comments",
            "post_id": post_id,
            "ai_requests": 0,
        }

    # Find our latest contribution.
    latest_self_index = -1
    ...
    # Only reply to an external contribution after our latest comment.
    candidates = [
        item
        for index, item in enumerate(normalized)
        if index > latest_self_index and not item["is_self"]
    ]

    if not candidates:
        return {
            "status": "no_action",
            "reason": "no_new_external_reply",
            "post_id": post_id,
            "ai_requests": 0,
            "latest_self_index": latest_self_index,
        }

    latest_external = candidates[-1]

    thread_lines = []

    for item in normalized[-20:]:
        role = "OUR AGENT" if item["is_self"] else "OTHER AGENT"

        thread_lines.append(
            f"[{role}] {item['author']}: {item['content']}"
        )

    thread_text = "\n\n".join(thread_lines)

    # Exactly one AI generation call.
    reply = await _v42_6_openrouter_reply(thread_text)

    # Post the reply as a Moltbook comment.
    post_status, published = await _v42_6_moltbook_request(
        "POST",
        f"{MOLTBOOK_API_BASE}/posts/{post_id}/comments",
        {
            "content": reply,
    ...
    if post_status >= 400:
        raise HTTPException(
            status_code=502,
            detail={
                "message": "Moltbook comment post failed",
    ...
    verification = await _v42_6_verify_comment_response(
        published
    )
    ...
    return {
        ...
        "version": "V42.6",
        "post_id": post_id,
        "reply_to_comment_id": latest_external.get("id"),
        "reply_to_author": latest_external.get("author"),
        "reply": reply,
        "ai_requests": 1,
        "verification": verification,
    }

# ================= END V42.6 AI THREAD REPLY =================
